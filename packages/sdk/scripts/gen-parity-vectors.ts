...
/**
 * The constants both implementations must agree on, and the form they must agree in.
 *
 * Written checksummed on purpose. The first run of these vectors against the Kotlin
 * client found a divergence nobody had noticed: the SDK emits EIP-55 checksummed
 * addresses, the port emitted lowercase. The chain does not care, and the one place
 * the two were compared happened to ignore case, so nothing was broken -- but the two
 * clients were naming the same box differently, and the next comparison that forgot
 * `ignoreCase` would have been silently wrong. Checksum is also what protects a user
 * reading an address off a screen, which is the whole premise of this product.
 *
 * Comparing the constants alone would not have caught it: both files held identical
 * strings. Only pinning derived output in its exact rendered form does.
 */
const chain = {
  chainId: ARC_TESTNET_CHAIN_ID,
  cctpDomain: ARC_CCTP_DOMAIN,
  usdcDecimals: USDC_DECIMALS,
  maxLogRange: Number(MAX_LOG_RANGE),
  deployBlocks: {
    ctrlArcZ: Number(CTRL_ARCZ_DEPLOY_BLOCK),
    stealthAnnouncer: Number(STEALTH_ANNOUNCER_DEPLOY_BLOCK),
  },
  // getAddress() rather than the literals, so this records the canonical rendering
  // and not merely what someone typed into the source file.
  addresses: {
    usdc: getAddress(ADDRESSES.USDC),
    eurc: getAddress(ADDRESSES.EURC),
    memo: getAddress(ADDRESSES.MEMO),
    permit2: getAddress(ADDRESSES.PERMIT2),
    multicall3: getAddress(ADDRESSES.MULTICALL3),
    multicall3From: getAddress(ADDRESSES.MULTICALL3_FROM),
    ctrlArcZ: getAddress(CTRL_ARCZ_ADDRESS),
    codeClaimVerifier: getAddress(CODE_CLAIM_VERIFIER_ADDRESS),
    spendPolicyFactory: getAddress(SPEND_POLICY_FACTORY_ADDRESS),
    spendPolicyAccountImpl: getAddress(SPEND_POLICY_ACCOUNT_IMPL_ADDRESS),
    shieldVault: getAddress(SHIELD_VAULT_ADDRESS),
    stealthAnnouncer: getAddress(STEALTH_ANNOUNCER_ADDRESS),
  },
};
