...
| **Tests**      | 742 in total: 123 Foundry, 384 SDK, 146 demo-kit, 56 api, 33 keeper, plus live testnet runs                                                        |
...
The Android source is not public, which is exactly why the vectors are not a convenience here. [`docs/android/ParityVectorsTest.kt`](./docs/android/ParityVectorsTest.kt) is a byte-identical copy of the Kotlin test that runs in that project: no application code, no keys, no endpoints, only the assertions that hold the two implementations to one specification. The part you cannot read is pinned to the part you can. [What it covers, and how to check it](./docs/android/README.md).
...
`packages/sdk/parity-vectors.json`, and the Kotlin test that does the holding is
published here: [`docs/android/ParityVectorsTest.kt`](./docs/android/ParityVectorsTest.kt).
See [Two clients: web and Android](#two-clients-web-and-android).
...
**Create a subscription.** Pick the merchant from a list rather than typing its name, point it at an address, then say the two things anyone actually has in mind: how much each charge is, and how many of them. The name is picked because it is not local trivia: it is packed into the box's stealth announcement, so a typo would follow the box to every other device. The budget is shown rather than asked, because it is an answer, and it is shown beside what Circle charges to move it into the box and what the two come to together. Authorising a subscription is a payment, and it was the last payment screen in the app that did not say what would leave the wallet.
...
verdict and the send is decided on the rules alone. The screen says so rather
than hiding it: the deep check reports "could not be reached" beside the rule
result, because the half of the firewall that did not run is the half worth
knowing about.
